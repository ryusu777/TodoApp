...
public class GetProjectByIdQueryTest : BaseFunctionalTest
{
    public GetProjectByIdQueryTest(WebAppFactory factory) : base(factory)
    {
    }
    ...
    public async Task Should_ReturnSuccess_WhenDataIsValid()
    {
        ...
        // Act
        var result = await _sender.Send(new GetProjectByIdQuery("ProjectA"), CancellationToken.None);
    ...
    [Fact]
    public async Task Should_ReturnFailure_WhenProjectIsNotFound()
    {
        // Arrange
        using var scope = Factory.Services.CreateAsyncScope();
        ISender _sender = scope.ServiceProvider.GetRequiredService<ISender>();
        ...
        // Act
        var result = await _sender.Send(new GetProjectByIdQuery("ProjectC"), CancellationToken.None);
        // Asssert
